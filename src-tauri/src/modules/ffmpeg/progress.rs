...
impl Progress {
    fn parse_value<T: std::str::FromStr>(value: &str) -> Result<T, ProgressParseError> {
        value
            .trim()
            .parse::<T>()
            .or(Err(ProgressParseError::UnableToParseValue(
                value.to_string(),
            )))
    }
    ...
    fn set(&mut self, key: &str, value: &str) -> Result<(), ProgressParseError> {
        ...
        match key {
            ...
            "out_time_us" => {
                self.out_time =
                    Some(Duration::from_micros(Progress::parse_value(value)?).as_secs_f64())
            }
            "out_time_ms" => (),
            "out_time" => (),
            "bitrate" => (),
    ...
    pub fn try_parse(&mut self, str: &str) -> Result<(), ProgressParseError> {
        ...
        let key = key_value_split
            .next()
            .ok_or(ProgressParseError::KeyNotFound)?;
        let value = key_value_split
            .next()
            .ok_or(ProgressParseError::ValueNotFound)?;
        self.set(key, value)?;
        Ok(())
    }
}
