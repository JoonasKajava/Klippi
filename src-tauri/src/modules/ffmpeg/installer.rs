...
pub fn uncompress_with_new_name(archive: PathBuf, new_name: &str) -> Result<()> {
    ...
    let destination = archive
        .parent()
        .context("Unable to get parent folder of file")?;
    ...
    let first_file = file_list
        .first()
        .context("Unable to get first file from archive")?;
    ...
    if uncompressed_folder.is_dir() {
        ...
        println!(
            "Renaming {:?} to {:?}",
            &uncompressed_folder, &full_new_name
        );
...
pub async fn install_ffmpeg(window: Window, install_location: &PathBuf) -> Result<String> {
    let downloaded_file = download(
        PathBuf::from(FFMPEG_DOWNLOAD_URL),
        install_location.to_path_buf(),
        |progress| {
            window
                .emit("download_progress", progress)
                .context("Unable to emit progress")
