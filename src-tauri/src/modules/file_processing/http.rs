...
pub async fn get_available_filename(file_path: &PathBuf) -> Result<PathBuf> {
    ...
    let original_name = file_path
        .file_stem()
        .context("Unable get file stem from file path")?
        .to_string_lossy();
...
pub async fn download<F: Fn(DownloadProgress)>(
    ...
    if parsed_target.extension().is_none() {
        let url_filename = url.file_name().context(
            "Unable to determinate filename since url or target do not contain filename",
        )?;
    ...
    fs::create_dir_all(
        parsed_target
            .parent()
            .context("Unable to folder from filename")?,
    )?;
    ...
    let res = client.get(url.to_string_lossy().to_string()).send().await?;
    ...
    let original_extension = &target_file
        .extension()
        .context("Unable to get file extension")?;
    let temp_filename = &target_file.with_extension(format!(
        "{}.{}",
