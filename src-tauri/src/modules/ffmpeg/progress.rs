...
impl Progress {
    fn parse_value<T : std::str::FromStr>(value: &str) -> Result<T, ProgressParseError> {
        value.parse::<T>().or(Err(ProgressParseError::UnableToParseValue(value.to_string())))
    }
    ...
    fn set(&mut self, key: &str, value: &str) -> Result<(), ProgressParseError> {
        match key {
            ...
            "out_time_us" => self.out_time = Some(Duration::from_micros(Progress::parse_value(value)?)),
            "out_time_ms" => (),
            "out_time" => (),
            "bitrate" => (),
            "stream_0_0_q" => (),
