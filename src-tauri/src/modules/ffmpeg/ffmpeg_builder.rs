...
impl<'a> FFmpegBuilder {
    ...
    pub fn input(&mut self, input: File) -> Result<&mut Self> {
        if !input.path.try_exists()? {
            bail!(
                "Trying to add file({}) that doesn't exist into ffmpeg",
                input.path.display()
            );
        }
    ...
    pub fn video_filter(&mut self, key: impl Into<String>, value: impl Into<String>) -> &mut Self {
        self.video_filters
            .push(Param::Pair(key.into(), value.into()));
        self
    }
    ...
    pub fn to_command(&self) -> Command {
        ...
        if !self.video_filters.is_empty() {
            command.args([
                "-vf",
                self.create_filter_string(&self.video_filters).as_str(),
            ]);
        }
        if !self.audio_filters.is_empty() {
            command.args([
                "-filter:a",
                self.create_filter_string(&self.audio_filters).as_str(),
            ]);
        }
